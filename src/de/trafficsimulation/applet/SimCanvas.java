...
*/
  ...
  public SimCanvas(int choice_Szen, double density, double qIn, double perTr,
      double p_factor, double deltaB, double p_factorRamp, double deltaBRamp,
      int tsleep_ms) {
    ...
    microstreet = new MicroStreet(2 * Math.PI * RADIUS_M, density, p_factor,
        deltaB, fcnr, choice_Szen);
  ...
  public void setScales() {
    ...
    if (false) {
      System.out.println("SimCanvas.setScales:" + " xsize_m=" + xsize_m
          + " xsize=" + xsize + " ysize=" + ysize + " xstart=" + xstart
          + " ystart=" + ystart + ", l_straight_m=" + l_straight_m
          + ", rdlen_m=" + rdlen_m);
    }
    ...
    dia1x = (isRingroad) ? (int) (0.99 * xsize) - 2 * diaWidth
        : (int) (0.80 * xsize) - 2 * diaWidth;
    dia2x = dia1x + diaWidth;
    dia3x = dia1x;
    dia4x = dia2x;

    dia1y = (isRingroad) ? (int) (0.80 * ysize) - 2 * diaHeight
  ...
  public void run() {
    ...
    while (Thread.currentThread() == runner) {
      ...
      synchronized (microstreetLock) {
        ...
        microstreet.update(time, TIMESTEP_S, choice_Szen, density, qIn, perTr,
            p_factor, deltaB);
        if (choice_Szen == 2) {
          onramp.update(time, TIMESTEP_S, qRamp, perTr, p_factorRamp,
              deltaBRamp);
        }
        if (SHOW_INSET_DIAG) {
          updateInsetDiagrams();
      ...
      tsleepFrame_ms = (int) ((tsleep_ms - simTime_ms - prepTime_ms)
          / framesPerStep - avgPaintTime_ms + dtf_ms);
      ...
      if (false) {
        System.out.println("tsleep=" + tsleep_ms + ", tsleepFr="
            + tsleepFrame_ms + ", simT=" + simTime_ms + ", prepT="
            + prepTime_ms + ", avgPaintT=" + avgPaintTime_ms + ", totPaintT="
  ...
  public void start(int choice_Szen, double density) {
    System.out.println("SimCanvas.start(choice_Szen=" + choice_Szen
        + ", density=" + density + ")");
    microstreet = new MicroStreet(rdlen_m, density, p_factor, deltaB, fcnr,
        choice_Szen);
    if ((choice_Szen == 3) || (choice_Szen == 4) || (choice_Szen == 5)) {
      imposeSpeedLimit();
    }
    if (choice_Szen == 2) {
      double mergingpos = (STRAIGHT_ROAD) ? ramppos_m : Math.PI * RADIUS_M
          + l_straight_m + 0.5 * L_RAMP_M;
      System.out.println("SimCanvas.start: mergingpos=" + mergingpos
          + ",l_straight_m=" + l_straight_m + ",total_rmplen_m="
          + total_rmplen_m);
      onramp = new OnRamp(microstreet, total_rmplen_m, L_RAMP_M, mergingpos,
          p_factorRamp, deltaBRamp);
    }
  ...
  private void paintCircleRoad(Graphics g) {
    ...
    for (int i = NPOINTS + 1; i < 2 * NPOINTS + 2; i++) {
      DetPosC dp = new DetPosC(rdlen_m * (i - 2 * NPOINTS - 1) / NPOINTS,
          -LANEWIDTH_M);
      xpoints[i] = dp.x;
      ypoints[i] = dp.y;
    }
  ...
  private void updateCircle(Graphics g, int framesPerStep) {
    ...
    for (int iframes = 1; iframes <= framesPerStep; iframes++) {
      ...
      for (int i = imax; i >= 0; i--) {
        PolyVehCircle pvc = new PolyVehCircle(weightNew * pos[i]
            + (1. - weightNew) * old_pos[i], weightNew * lane[i]
            + (1. - weightNew) * old_lane[i], lengthVeh[i], VEH_WIDTH_M, r1,
            r2, xc, yc);

        // again, old veh a little big larger -> initpos-=1, length+=2
        PolyVehCircle pvc_old = new PolyVehCircle(weightOld * pos[i]
            + (1. - weightOld) * old_pos[i] - 1, weightOld * lane[i]
            + (1. - weightOld) * old_lane[i], lengthVeh[i] + 2,
            VEH_WIDTH_M + 2, r1, r2, xc, yc);
  ...
  private void paintStraight(Graphics g) {
    ...
    g.fillRect(xstart, ystart - 2 * lw, (int) rdlen, 2 * lw); // left-upper pt,
                                                              // obj size
    ...
    if (choice_Szen == 2) {
      ...
      // access road
      int[] poly2_x = { xstart, xstart, x_merge_end - lRamp,
          x_merge_end - lRamp };
  ...
  private void updateU(Graphics g, int framesPerStep) {
    ...
    for (int iframes = 1; iframes <= framesPerStep; iframes++) {
      ...
      for (int i = imax; i >= 0; i--) {
        PolyVehU pvu = new PolyVehU(weightNew * pos[i] + (1. - weightNew)
            * old_pos[i], weightNew * lane[i] + (1. - weightNew) * old_lane[i],
            lengthVeh[i], VEH_WIDTH_M, r1, r2, xc, l_straight, yc,
            (int) (rdlen));

        PolyVehU pvu_old = new PolyVehU(weightOld * pos[i] + (1. - weightOld)
            * old_pos[i] - 1, weightOld * lane[i] + (1. - weightOld)
            * old_lane[i], lengthVeh[i] + 2, VEH_WIDTH_M + 2, r1, r2, xc,
  ...
  private void updateStraight(Graphics g, int framesPerStep) {
    ...
    for (int iframes = 1; iframes <= framesPerStep; iframes++) {
      ...
      for (int i = imax; i >= 0; i--) {
        PolyVehStraight pvs = new PolyVehStraight(weightNew * pos[i]
            + (1. - weightNew) * old_pos[i], weightNew * lane[i]
            + (1. - weightNew) * old_lane[i], lengthVeh[i], VEH_WIDTH_M,
            (int) (rdlen));

        PolyVehStraight pvs_old = new PolyVehStraight(weightOld * pos[i]
            + (1. - weightOld) * old_pos[i] - 1, weightOld * lane[i]
            + (1. - weightOld) * old_lane[i], lengthVeh[i] + 2,
            VEH_WIDTH_M + 2, (int) (rdlen));
      ...
      if (choice_Szen == 2) {
        ...
        for (int i = imaxOn; i >= 0; i--) {
          PolyVehStraightRamp pvs = new PolyVehStraightRamp(weightNew
              * posOn[i] + (1. - weightNew) * old_posOn[i], lengthVehOn[i],
              VEH_WIDTH_M, (int) (total_rmplen));

          PolyVehStraightRamp pvs_old = new PolyVehStraightRamp(weightOld
              * posOn[i] + (1. - weightOld) * old_posOn[i] - 1,
  ...
  private void prepareVehData() {
    ...
    if (circleCarsRemoved) {
      System.out.println("circleCarsRemoved!");
    }
  ...
  private void drawLines(Graphics g) {
    ...
    if (choice_Geom == 0) {
      for (int i = 0; i < numberOfLines; i++) {
        ...
        PolyVehCircle roadLine = new PolyVehCircle(pos_m, -1, lineLength,
            lineWidth, r1, r2, xc, yc);
    ...
    if (choice_Geom == 1) {
      for (int i = 0; i < numberOfLines; i++) {
        double pos_m = i * (LINELENGTH_M + GAPLENGTH_M);
        PolyVehU roadLine = new PolyVehU(pos_m, -1, lineLength, lineWidth, r1,
            r2, xc, l_straight, yc, (int) (rdlen));
        g.fillPolygon(roadLine.xPoints, roadLine.yPoints, 4);
      }
      if (choice_Szen == 2) {
    ...
    if (choice_Geom == 2) { // STRAIGHT_ROAD=true
      for (int i = 0; i < numberOfLines; i++) {
        double pos_m = i * (LINELENGTH_M + GAPLENGTH_M);
        PolyVehStraight roadLine = new PolyVehStraight(pos_m, -1, lineLength,
            lineWidth, (int) (rdlen));
        g.fillPolygon(roadLine.xPoints, roadLine.yPoints, 4);
      }
      if (choice_Szen == 2) {
        ...
        for (int i = 0; i < numberOfLines; i++) {
          int xLeft = ramppos
              + (int) (scale * (-0.5 * L_RAMP_M + i
                  * (LINELENGTH_M + GAPLENGTH_M)));
          // drawPolygon draws centered lines -> must add half linewidth
          // to height,width parameters of drawRect
  ...
  public void newValues(int choice_Szen, double density, double qIn,
      double perTr, double v0_limit, double p_factor, double deltaB,
      int tsleep_ms) {
    ...
    if ((STRAIGHT_ROAD) && (choice_Szen == 2)) {
      choice_Geom = 2;
    }
    setScales(); // NOT after update of rdlen, but behind choice_Szen
    rdlen_m = (choice_Geom == 0) ? 2 * Math.PI * RADIUS_M : Math.PI * RADIUS_M
        + 2 * l_straight_m;

    System.out.println("SimCanvas.newValues():" + " STRAIGHT_ROAD="
        + STRAIGHT_ROAD + " choice_Szen= " + choice_Szen + " choice_Geom= "
        + choice_Geom);

  }

  public void newValues2(double qIn, double perTr, double p_factor,
      double deltaB, double qRamp, double p_factorRamp, double deltaBRamp,
      int tsleep_ms) {
    ...
    rdlen_m = (choice_Geom == 2) ? STRAIGHT_RDLEN_M : Math.PI * RADIUS_M + 2
        * l_straight_m;
    System.out.println("SimCanvas.newValues2():" + " STRAIGHT_ROAD="
        + STRAIGHT_ROAD + " choice_Szen= " + choice_Szen + " choice_Geom= "
        + choice_Geom);
  }
  ...
  private void imposeSpeedLimit() {
    ...
    double v0trucks = (v0_limit < VMAX_TRUCK_KMH / 3.6) ? v0_limit
        : VMAX_TRUCK_KMH / 3.6;
  ...
  private void paintLegend(Graphics g) {
    ...
    int xoff = (withProbeCar) ? metricsText.stringWidth(probeString) + pkwLen
    ...
    g.drawString((choice_Szen == 6) ? idString : truckString, x0, y0
        + lineHeight);
    if (withProbeCar) {
  ...
  /*
   * // alte PaintLegend. Loeschen, wenn die neue "zeitgetestet" private void
   * paintLegend(Graphics g){
   * 
   * // boolean withProbeCar=((choice_Szen==5)||(choice_Szen==1)); boolean
   * withProbeCar=false; String carString = (GERMAN) ? "PKW" : "Car"; String
   * truckString = (GERMAN) ? "LKW" : "Truck"; String idString = (GERMAN) ?
   * "Gleicher PKW-Typ" : "Same Car"; String probeString = (GERMAN) ?
   * "Testfahrzeug" : "Probe Car";
   * 
   * int lineHeight=(int)(1.25*metricsText.stringWidth("M")); int
   * x0=(int)(0.025*xsize); // xsize,ysize in pixels int y0=yc+r2-3*lineHeight;
   * if (choice_Szen==6){y0+=lineHeight;} if(choice_Geom==2){ y0=ystart -
   * 4*lineHeight;}
   * 
   * int xoff=(withProbeCar) ? metricsText.stringWidth(probeString)+pkwLen :
   * metricsText.stringWidth(truckString)+pkwLen; if (choice_Szen==6){
   * xoff=metricsText.stringWidth(idString)+pkwLen;}
   * 
   * // text "Cars" or "Trucks"
   * 
   * g.setFont(textFont); g.setColor(Color.black);
   * 
   * g.drawString(carString,x0,y0); g.drawString(
   * (choice_Szen==6)?idString:truckString, x0,y0+lineHeight); if
   * (withProbeCar){ g.drawString(probeString,x0, y0+2*lineHeight); }
   * 
   * // vehicle symbols
   * 
   * g.setColor(Color.red); g.fillRect (x0+xoff, y0 -vehWidth, pkwLen,
   * vehWidth); if (choice_Szen==6){ g.setColor(Color.green); g.fillRect
   * (x0+xoff, y0+lineHeight -vehWidth, pkwLen, vehWidth);} else{
   * g.setColor(Color.black); g.fillRect (x0+xoff, y0+lineHeight -vehWidth,
   * lkwLen, vehWidth);} if (withProbeCar){ g.setColor(Color.blue);
   * g.fillRect(x0+xoff, y0+2*lineHeight-vehWidth, pkwLen, vehWidth); } }
   */
  ...
  private void paintTimes(Graphics g) {
    ...
    String timeString = str_time + (new Integer(min)).toString()
    ...
    g.setColor(backgrColor);
    // g.setColor(Color.white);
    g.fillRect(x0 + widthTime, y0 - textHeight, widthDisplay - widthTime,
        textHeight);
  ...
  private void paintSpeedLimit(Graphics g) {
    ...
    if (v0_kmh >= V0_LIMIT_MAX_KMH) { // no speed limit
      ...
      for (int i = -2; i <= 2; i++) {
        g.drawLine(xc - (int) (0.7 * radiusSS) + i * space, yc
            + (int) (0.7 * radiusSS) + i * space, xc + (int) (0.7 * radiusSS)
            + i * space, yc - (int) (0.7 * radiusSS) + i * space);
      }
    }
  ...
  class PolyVehCircle {
    ...
    public PolyVehCircle(double pos, double lane, double length, double width,
        int r1, int r2, int xc, int yc) {
      ...
      double r = (lane >= 0) ? r1 + (0.25 + 0.5 * lane) * (r2 - r1) : r1 + 0.5
          * (r2 - r1);
  ...
  class PolyVehStraight {
    ...
    public PolyVehStraight(double pos_m, double lane, double vehLen_m,
        double vehWidth_m, int rdlen) {
  ...
  class PolyVehStraightRamp {
    ...
    public PolyVehStraightRamp(double pos_m, double vehLen_m,
        double vehWidth_m, int rdlen) {
      ...
      if (pos_m > (ramppos_m - 0.5 * L_RAMP_M)) { // actual merging section
        ...
        double xveh = xstart + scale
            * (xs_m - ca * (xs_m - pos_m - 0.5 * vehLen_m));
        double yveh = 0.5 * lw + scale
            * (ys_m + sa * (xs_m - pos_m - 0.5 * vehLen_m));
  ...
  private void setInsetDiagrams() {
    ...
    if (choice_Szen == 1) {
      ...
      fcdxdv = new StatBufDiag(-10.0, 10.0, 0.0, 100.0, diaWidth, diaHeight,
          400, Color.white, Color.black, diagram1, "Velocity Difference",
          "Headway", "", "Blue Car", 5.0, "5 m/s", 50.0, "50 m");
      String[] ylabels = { "acc", "V" };
      Color[] colors = { Color.black, Color.red };
      double[] ytics = { 4.0, 2.86 };
      String[] yticLabels = { "4 m/s^2", "20 m/s" };
      fcvt = new DynBufDiag(0.0, 60.0, -5.0, 5.0, diaWidth, diaHeight, 60.0,
          Color.white, Color.black, diagram2, "time", ylabels, colors, "",
          "Acceleration+Velocity", 30.0, "-30s", ytics, yticLabels);
      Qrho = new StatBufDiag(0.0, 0.15, 0.0, 0.8, diaWidth, diaHeight, 2000,
          Color.white, Color.black, diagram3, "rho", "Q", "Detectors D1-D4",
          "Data from", 0.075, "75 Veh/km", 0.417, "1500 Veh/h");
      QrhoS = new StatBufDiag(0.0, 0.15, 0.0, 0.8, diaWidth, diaHeight, 1000,
          Color.white, Color.black, diagram4, "rho", "Q", "Detectors D1-D4",
          "Actual Data", 0.075, "75 Veh/km", 0.417, "1500 Veh/h");
    ...
    if (choice_Szen == 5) {
      ...
      double[] ytics = { 4.0, 2.86 };
      String[] yticLabels = { "4 m/s^2", "20 m/s" };
      fcvt = new DynBufDiag(0.0, 120.0, -5.0, 5.0, diaWidth, diaHeight, 120.0,
          Color.white, Color.black, diagram1, "time", ylabels, colors,
          "Blue Car", "Acceleration+Velocity", 60.0, "-60s", ytics, yticLabels);
      String[] ylabels1 = { "Q" };
      Color[] colors1 = { Color.black };
      double[] ytics1 = { 0.417 };
      String[] yticLabels1 = { "1500 Veh/h" };

      Qt = new DynBufDiag(0.0, 60.0, 0.0, 0.8, diaWidth, diaHeight, 60.0,
          Color.white, Color.black, diagram2, "time", ylabels1, colors1, "",
          "Total Flow at D1", 30.0, "-30s", ytics1, yticLabels1);
    }
    ...
    if ((choice_Szen == 2) || (choice_Szen == 3) || (choice_Szen == 4)) {
      ...
      Qrho = new StatBufDiag(0.0, 0.15, 0.0, 0.8, diaWidth, diaHeight, 500,
          Color.white, Color.black, diagram1, "rho", "Q", "Detector at D2",
          "Measured Data", 0.075, "75 Veh/km", 0.417, "1500 Veh/h");
      ...
      if (false) {
        ...
        Qt = new DynBufDiag(0.0, 0.15, 0.0, 0.8, diaWidth, diaHeight, 60.0,
            Color.white, Color.black, diagram1, "time", ylabels1, colors1, "",
            "Total Flow at D1", 30.0, "-30s", ytics1, yticLabels1);
      }

      QrhoS = new StatBufDiag(0.0, 0.15, 0.0, 0.8, diaWidth, diaHeight, 100,
          Color.white, Color.black, diagram2, "rho", "Q", "Detector at D2",
          "Actual Data", 0.075, "75 Veh/km", 0.417, "1500 Veh/h");
      Qrho1 = new StatBufDiag(0.0, 0.15, 0.0, 0.8, diaWidth, diaHeight, 500,
          Color.white, Color.black, diagram3, "rho", "Q", "Detector at D1",
          "Measured Data", 0.075, "75 Veh/km", 0.417, "1500 Veh/h");
      QrhoS1 = new StatBufDiag(0.0, 0.15, 0.0, 0.8, diaWidth, diaHeight, 100,
          Color.white, Color.black, diagram4, "rho", "Q", "Detector at D1",
          "Actual Data", 0.075, "75 Veh/km", 0.417, "1500 Veh/h");
