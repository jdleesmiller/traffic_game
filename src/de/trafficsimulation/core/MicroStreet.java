  ...
  static final int RIGHT=1; 
	...
	if(choice_Szen==1){
		...
		street.insertElementAt
		    (new Car(getRoadLength()-(i+1)*distance,vNew,lane,
			     getIdmCar(),polite,PKW_LENGTH_M, colorCar,i),i);
  	...
  	if(choice_Szen==6){
		...
		if (i%7==0){
		    if (i!=49)	street.insertElementAt(
		       new Obstacle(getRoadLength()-(i+1)*20.0, lane, 5., 9999),index);
		    if(i==49)   street.insertElementAt(
		       new Obstacle(getRoadLength()+30.0-(i+1)*20.0, lane, 5., 9999),index);
		}
		else{
		    if (i<((int)(0.5*nCars))){
		    street.insertElementAt
			(new Car(getRoadLength()-(i+1)*20.0,0.0,lane,
				 getIdmCar(), inconsiderate, PKW_LENGTH_M, colorCar, i),index);
		    }
		    else{
			street.insertElementAt
			(new Car(getRoadLength()-(i+1)*20.0,0.0,lane,
				 getIdmCar(), inconsiderate, PKW_LENGTH_M, Color.green,i),index);
		    }
		}
		if (i!=49){ index++; }
	    }
	...
	if (choice_Szen==5){
	    ...
	    street.addElement
	      (new Car(5.0,getIdmCar().Veq(getRoadLength()),0, getIdmCar(), polite, 
	...
	if(choice_Szen==3){
	    ...
	    street.insertElementAt
	      (new Car(5.0, getIdmTruck().Veq(getRoadLength()), 0, getIdmTruck(), polite, 
	...
	if(choice_Szen==4){
	    ...
	    street.insertElementAt
	      (new Car(5.0, getSync1Car().Veq(getRoadLength()), 0, getSync1Car(), polite, 
...
// <martin nov07>
    ...
    public double length(){ return getRoadLength(); }
    ...
    protected Vector<Double> setDistances(){  // neglect gaps in front of first/last veh
...
*/
	...
	double downLeftPos=(i_ld>-1) 
	    ? ((Moveable)(street.elementAt(i_ld))).position():getRoadLength();
	double downRightPos=(i_rd>-1) 
	    ? ((Moveable)(street.elementAt(i_rd))).position():getRoadLength(); 
	double downLeftVel=(i_ld>-1) 
	    ? ((Moveable)(street.elementAt(i_ld))).velocity():getRoadLength();    
	double downRightVel=(i_rd>-1) 
	    ? ((Moveable)(street.elementAt(i_rd))).velocity():getRoadLength();

	if (choice_BC==0){ // periodic BC
	
	    street.insertElementAt
		(new BCCar(upLeftPos+getRoadLength(), upLeftVel, 0, getIdmCar(), PKW_LENGTH_M),0);
	    street.insertElementAt
	        (new BCCar(upRightPos+getRoadLength(), upRightVel, 1, getIdmCar(), PKW_LENGTH_M),0);
	    int imax=street.size();
	    street.insertElementAt
		(new BCCar(downLeftPos-getRoadLength(), downLeftVel, 0, getIdmCar(), PKW_LENGTH_M),imax);
	    imax++;
	    street.insertElementAt
		(new BCCar(downRightPos-getRoadLength(), downRightVel, 1, getIdmCar(), PKW_LENGTH_M),imax);
	}
        if (choice_BC==1){ // open BC
            double dx = 200.;  // distance of the boundary cars
	    street.insertElementAt
		(new BCCar(downLeftPos+dx, downLeftVel, 0, getIdmCar(), PKW_LENGTH_M),0);
	    street.insertElementAt
		(new BCCar(downRightPos+dx, downRightVel, 1, getIdmCar(), PKW_LENGTH_M),0);
	    int imax=street.size();
	    street.insertElementAt
		(new BCCar(upLeftPos-dx, upLeftVel, 0, getIdmCar(), PKW_LENGTH_M),imax);
	    imax=street.size();
	    street.insertElementAt
		(new BCCar(upRightPos-dx, upRightVel, 1, getIdmCar(), PKW_LENGTH_M),imax);
	}
    }
	...
	if (nin>1.0){  // new vehicle imposed by the inflow BC
	   ...
	   int i_ru=lastIndexOnLane(1);
		...
		MicroModel carmodel  =(choice_Szen!=4) 
                            ? getIdmCar() : getSync1Car();
		MicroModel truckmodel=(choice_Szen!=4) 
                            ? getIdmTruck() : getSync1Truck();
		double     rand      = random.nextDouble()*1.0;
		int        randInt   = Math.abs(random.nextInt());
		...
		street.insertElementAt
	     ...
	     MicroModel carmodel  =(choice_Szen!=4) ? getIdmCar() : getSync1Car();
	     MicroModel truckmodel=(choice_Szen!=4) ? getIdmTruck() : getSync1Truck();
	     double     rand      = random.nextDouble()*1.0;
	     int        randInt   = Math.abs(random.nextInt());
